        ...
        let
          ...
          launchSimula = ''
            ...
            if grep -qi NixOS /etc/os-release; then
              ...
              NIXPKGS_ALLOW_UNFREE=1 nix run --impure github:nix-community/nixGL#nixGLIntel -- ${pkgs.util-linux}/bin/script -qfc "$GODOT_BINARY -m \"$PROJECT_PATH\" $@" >(${pkgs.coreutils}/bin/stdbuf -oL -eL ${pkgs.colorized-logs}/bin/ansi2txt > "$SIMULA_DATA_DIR/log/output.file")
            fi
          '';
